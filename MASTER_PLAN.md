...
- Product built and tested: `bidoro.html` (the real, sellable standalone app) and `bidoro-demo.jsx` (Claude.ai-only demo version)
- Runs on the buyer's own free OpenRouter key — genuinely $0 to operate, for you and them
- `bidoro-openrouter-setup.pdf` and `bidoro-quickstart.pdf` — buyer-facing setup docs
